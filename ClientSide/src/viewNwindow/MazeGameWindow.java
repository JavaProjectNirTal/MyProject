...
public class MazeGameWindow extends GameWindow {
	...
	void initWidgets() { // change!!!!!! without all if!!! use FACTORY
		...
		btnStart.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (board != null)
					board.dispose();
				setBoard(new MazeGameBoard(shell, SWT.BORDER, s));
		...
		if (board != null) {
			board.addKeyListener(new KeyListener() {
				...
				public void keyPressed(KeyEvent k) {
					mazeCharacterState = ((MazeGameBoard) board)
							.getGameCharacter(); // (x,y) before move
					switch (k.keyCode) {
	 ...
	 * 
	 * board.dispose();
	 * ((MazeGameBoard)board).setGameCharacter(this.getDescription());
	 * buildBoard(); }
	 * 
	 * private void buildBoard() { board.moveBelow(btnStart);
	 * board.setLayoutData(new GridData(SWT.FILL, SWT.FILL, true, true, 1, 2));
	 * shell.layout(); board.setFocus();
	 * 
	 * board.addKeyListener(new KeyListener() {
	 * 
	 * @Override public void keyReleased(KeyEvent k) { }
	 * 
	 * @Override public void keyPressed(KeyEvent k) { mazeCharacterState =
	 * ((MazeGameBoard) board).getGameCharacter();
	 * 
	 * switch (k.keyCode) {
	 * 
	 * case SWT.ARROW_RIGHT: { setCommand("selectMoves: right " +
	 * mazeCharacterState); MazeGameWindow.this.setChanged();
	 * MazeGameWindow.this.notifyObservers(); updatePuzzle(); //
	 * //////////////////////////////////// break; }
	 * 
	 * case SWT.ARROW_LEFT: { setCommand("selectMovesPuzzle: left " +
	 * puzzleString); PuzzleGameWindow.this.setChanged();
	 * PuzzleGameWindow.this.notifyObservers(); updatePuzzle(); //
	 * //////////////////////////////////// break; }
	 * 
	 * case SWT.ARROW_UP: { setCommand("selectMovesPuzzle: up " + puzzleString);
	 * PuzzleGameWindow.this.setChanged();
	 * PuzzleGameWindow.this.notifyObservers(); updatePuzzle(); //
	 * //////////////////////////////////// break; } case SWT.ARROW_DOWN: {
	 * setCommand("selectMovesPuzzle: down " + puzzleString);
	 * PuzzleGameWindow.this.setChanged();
	 * PuzzleGameWindow.this.notifyObservers(); updatePuzzle(); //
	 * //////////////////////////////////// break; }
	 * 
